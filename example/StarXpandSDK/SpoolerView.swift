...
struct SpoolerView: View {
    ...
    @StateObject var spoolerViewModel = SpoolerViewModel()
    ...
    var body: some View {
        VStack {
            ...
            Button("Spool Print") {
                ...
                Task {
                    isProcessing = true
                    await spoolerViewModel.printing(starConnectionSettings)
                    isProcessing = false
                }
            }

            HStack {
                TextField("JobID", text: $spoolerViewModel.jobId)
                    .padding(1)
                    .textFieldStyle(.roundedBorder)
                ...
                Button("Get Job Status") {
                    ...
                    Task {
                        isProcessing = true
                        await spoolerViewModel.getJobStatus(starConnectionSettings)
                        isProcessing = false
                    }
                }
            }
            ...
            ScrollView {
                Text(spoolerViewModel.logText)
                    .padding()
            }

            Spacer()
        }
        .navigationTitle(Text("Printing -Spooler-"))
        .padding()
    }
}

@MainActor
class SpoolerViewModel: ObservableObject {
    ...
    func printing(_ starConnectionSettings: StarConnectionSettings) async {
        ...
        do {
            ...
            getJobId = try await printer.print(command: command, starSpoolJobSettings: jobSettings)
        ...
        print (log)
        print("jobId: \(getJobId)")
        logText = log
        jobId = "\(getJobId)"    }
    
    func getJobStatus(_ starConnectionSettings: StarConnectionSettings) async {
        var log = ""
        ...
        do {
            ...
            let jobStatus = try await printer.getSpoolJobStatus(jobId: Int(jobId) ?? 0)

            log = jobStatus.description.replacingOccurrences(of: ",", with: "\n")
        } catch let error {
            log = "Error: \(error)"
        }
        print (log)
    ...
    private func createCommand() -> String {
